...
# HyperCeiler-Mondrian

### Redmi K60 / POCO F5 Pro 的 MIUI 14 兼容版本
...
> [!IMPORTANT]
> 这是基于 [ReChronoRain/HyperCeiler](https://github.com/ReChronoRain/HyperCeiler)
> 制作的非官方设备专用版本，并非 HyperCeiler 官方发行版。

## Mondrian 兼容版本

- 目标设备：Redmi K60 / POCO F5 Pro（`mondrian`）
- 已验证系统：MIUI 14 Global `V14.0.6.0.TMNMIXM`、Android 13
- 版本：`2.5.156-mondrian5`（versionCode `4005`）
- Xposed 环境：Vector 2.2 + NeoZygisk

本版本针对上述系统的 SystemUI 做了兼容处理，包括：

- 支持隐藏辅助 Wi-Fi（`slave_wifi`）图标
- 修复状态栏实时网速 Hook
- 修复双排移动网络图标兼容性
- 修复双排图标启用后 VoLTE / VoWiFi 图标无法隐藏
- 增加横向 Pixel 风格电池图标，内部显示电量数字、右侧显示充电状态，并适配深浅色变化
...
项目继续遵循上游的 AGPL-3.0 许可证；原作者、贡献者与第三方项目声明均予以保留。

## 上游源码的原始支持范围

Android 14-15 的 HyperOS

> 上述内容是上游源码的原始说明。本分支额外维护并验证了前文列出的
> `mondrian`、MIUI 14 Global、Android 13 组合。
