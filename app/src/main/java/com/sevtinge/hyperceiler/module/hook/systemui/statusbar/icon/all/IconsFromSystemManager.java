...
public class IconsFromSystemManager extends BaseHook {
    ...
    public void init() {
        ...
        findAndHookMethodSilently(statusBarIconControllerImpl,
            "setIconVisibility",
            String.class, boolean.class,
            new MethodHook() {
                @Override
                protected void before(MethodHookParam param) {
                    if (shouldHideStatusBarSlot((String) param.args[0])) {
                        param.args[1] = false;
                    }
                }
            }
    ...
    private boolean shouldHideStatusBarSlot(String slotName) {
        if ("slave_wifi".equals(slotName)) {
            int mode = mPrefsMap.getStringAsInt("system_ui_status_bar_icon_wifi_slave", 0);
            return mode == 2 || mode == 11;
        }
        return checkSlot(slotName);
    }
